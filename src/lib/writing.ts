...
const writingModules = import.meta.glob<string>('../content/writing/*.md', {
  as: 'raw',
  eager: true,
})
...
const parseFrontmatter = (raw: string) => {
  ...
  for (const line of frontmatter.split(/\r?\n/)) {
    ...
    if (key === 'image') data.image = cleanValue
  }

  return { data, content }
}

export const writingItems: Array<WritingItem> = Object.entries(writingModules)
  .map(([path, raw]) => {
